...
static void pedestrian_from_pyobject(PyObject * o, Pedestrian * a)//
{
	...
	int mem_index;
	PyObject * group_list_obj;
	PyObject * mate_id;

	a->pedestrian_id			= double_from_attribute(o,"pedestrian_id");
	a->group_id					= (int)double_from_attribute(o,"group_id");
	...
	a->attractor_count			= double_from_attribute(o,"attractor_count");

	//set list of ids of group mates
	group_list_obj = PyDict_GetItemString(o, "friend_zone");

	for(mem_index=0; mem_index <a->attractor_count; mem_index++){
		mate_id =  PySequence_GetItem(group_list_obj, mem_index);
		a->group_list[mem_index] = PyFloat_AsDouble(mate_id);
	}
...
static PyObject * add_group_pedestrian(PyObject * self, PyObject * args)//
{
    ...
    group_id = (int)double_from_attribute(p_pedestrian,"group_id");
...
static void  calculate_pedestrian_repulsion_attraction(Pedestrian *a,int index, int level_rk, int population_count)//
{
	...
	for(j = 0; j < population_count; j++) {
		...
		same_group = is_same_group(a->group_list, a->attractor_count, group_pedestrians[j].pedestrian_id);
		if(same_group == 1) {	// if inside group id list of this pedestrian
...
static int is_same_group(double *group_list, double attractor_count, double pedestrian_id){

	int i;
	for(i=0; i<attractor_count; i++){
			if(group_list[i]==pedestrian_id){
				return 1;
			}
	}
...
static PyObject * group_pedestrian_a_property(PyObject * self, PyObject * args)//
{
	...
	} else if (strcmp(property, "attractor_count") == 0){
		 return PyFloat_FromDouble(group_pedestrians[i].attractor_count);

	} else if (strcmp(property, "group_list") == 0){
		 member_list = PyMem_Malloc(group_pedestrians[i].attractor_count *10);
		 strcpy(member_list, "");
		 for (member_index=0; member_index < group_pedestrians[i].attractor_count; member_index++){
			  temp_id = (int) group_pedestrians[i].group_list[member_index];
			  sprintf(str_temp_id, "%d", temp_id);
			  strcat(member_list,str_temp_id);
			  strcat(member_list, "-");
		 }

		return Py_BuildValue("s",member_list);
...
//this method aims to add additional group member to existed pedestrian
static PyObject* add_additional_group_member(PyObject * self, PyObject * args){

	int i,mem_index,population_count;
	int current_count,additional_count;
	double ped_id;
	
    PyObject * additional_data, * group_list_obj, * mate_id;
	PyArg_ParseTuple(args, "O:add_additional_group_member", &additional_data);

	population_count = 0;
		
	ped_id = double_from_attribute(additional_data,"ped_id");
	additional_count = (int)double_from_attribute(additional_data,"additional_count");
	group_list_obj = PyDict_GetItemString(additional_data, "friend_zone");
	...
	for(i=0; i < population_count;i++){
		if (group_pedestrians[i].pedestrian_id == ped_id) {
				  current_count = (int)(group_pedestrians[i].attractor_count);
				  for(mem_index=0; mem_index < additional_count; mem_index++){
						mate_id =  PySequence_GetItem(group_list_obj, mem_index);
						group_pedestrians[i].group_list[current_count+mem_index] = PyFloat_AsDouble(mate_id);
				  }
				  //update count for this guy
				  group_pedestrians[i].attractor_count = current_count + additional_count;
				  return PyFloat_FromDouble(1.0);
		}
	}
...
static PyObject * set_parameters(PyObject * self, PyObject * args)//
{
	...
	//set group_num
	group_num	= (int)double_from_attribute(o, "group");
...
static PyMethodDef ForceModelMethods[] = {
	...
	{"add_additional_group_member",add_additional_group_member, METH_VARARGS,
		"Set additional group members"},
	{"target_changed", target_changed, METH_VARARGS,
			"Change target in split corridor"},
	{NULL, NULL, 0, NULL}
};
